...
from app.schemas.energy import BuildingEnergySummary
...
@router.get("/buildings", response_model=list[BuildingEnergySummary], summary="Get building energy intelligence")
def list_building_energy(session: Annotated[Session, Depends(get_session)]):
    ...
    with write_transaction(session):
        return building_energy_summaries(session)
